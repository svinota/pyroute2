...
class IPRoute(Netlink):
    ...
    *Usage*
    ...
    def rule(self, command, table, priority=32000, rtype='RTN_UNICAST',
             rtscope='RT_SCOPE_UNIVERSE', family=AF_INET, src=None, src_len=None, dst=None, dst_len=None):
        '''
        Rule operations
        ...
        * src      - IP source for Source Based (Policy Based) routing's rule
        ...
        addr_len = {
                AF_INET6: 128,
                AF_INET:  32}[family]
        if(dst_len != None and dst_len >= 0 and dst_len <= addr_len):
            msg['dst_len'] = dst_len
        else:
            msg['dst_len'] = 0
        if(src_len != None and src_len >= 0 and src_len <= addr_len):
            msg['src_len'] = src_len
        else:
            msg['src_len'] = 0
        if src is not None:
