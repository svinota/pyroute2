...
# Geneve option masks
GENEVE_CLASS_FULL_MASK = 0xFFFF
GENEVE_TYPE_FULL_MASK = 0xFF
GENEVE_DATA_BYTE_MASK = 0xFF
...
def fix_msg(msg, kwarg):
    if FlowerArgs.PROTOCOL not in kwarg:
...
def _build_ip_attrs(kwarg, ip_key, mask_key, nla_map):
    ...
    ip_ver = IP_VERSION_6 if is_ipv6_addr(addr) else IP_VERSION_4
    ...
    if mask:
        ...
        default = DEFAULT_IPV6_MASK if ip_ver == IP_VERSION_6 else DEFAULT_IPV4_MASK
...
def _detect_eth_type(attrs):
    ipv4_nlas = set(IpVersion2Nla[IP_VERSION_4].values())
    ipv6_nlas = set(IpVersion2Nla[IP_VERSION_6].values())
    attr_keys = set([attr[0] for attr in attrs])
...
def _build_geneve_opts_mask(opts_str):
    _, _, opt_data = parse_geneve_opt(opts_str)

    mask_data = bytes(bytearray([GENEVE_DATA_BYTE_MASK] * len(opt_data)))
        ...
        'attrs': [
            [TCA_FLOWER_KEY_ENC_OPTS_GENEVE, {
                'attrs': [
                    ...
                    [TCA_FLOWER_KEY_ENC_OPT_GENEVE_DATA, mask_data],
                ]
            }]
        ]
    }
