...
from utils import get_simple_bpf_program, require_user, skip_if_not_supported
...
class BasicTest(object):
    ...
    def get_qdiscs(self):
        return [
            x for x in self.ip.get_qdiscs() if x['index'] == self.interface
        ]
    ...
    def test_filter(self):
        ...
        prs = [
            ...
            and (
                ...
                is not None
                or x.get_attr('TCA_OPTIONS').get_attr('TCA_U32_ACT')
                is not None
            )
        ][0]
...
class TestHfsc(BasicTest):
    ...
    def test_hfsc(self):
        # root queue
        self.ip.tc('add', 'hfsc', self.interface, handle='1:0', default='1:1')
        qds = self.get_qdiscs()
        assert len(qds) == 1
...
class TestHtb(BasicTest):
    ...
    def _test_htb(self):
        ...
        self.ip.tc(
            'add', 'sfq', self.interface, '10:', parent='1:10', perturb=10
        )
        self.ip.tc(
            'add', 'sfq', self.interface, 0x200000, parent=0x10020, perturb=10
        )
        qds = self.get_qdiscs()
